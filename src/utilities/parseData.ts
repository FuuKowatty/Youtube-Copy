...
export const parseData = async (items: any[]) => {
  //poushujemy do tablicy id kanalow i video
  try {
    ...
    const {
      //data.items z tego linku i to jest rowne channelsData
        ...
        if (channelImage) {
          parsedData.push({
            ...
            videoViews: convertRawViewstoString(
              videosData[index].statistics.viewCount //dlaczego nie moge sie od item.statiics odwolac??
            ),
